...
### Phase 0 から Phase 1（GUI）へ

**API 変更は予想されません**:
- コアライブラリ（`src/lib.rs`）は安定を維持
- Tauri コマンドは拡張される可能性があるが変更されない
- 既存のコードは引き続き動作する

**新機能（追加的）**:
- ビジュアルブロックビルダー（フロントエンドのみ）
- 自動 `_N:` アノテーション（フロントエンドのみ）

---

### Phase 0 から Phase N（ロジックチェック）へ
...
**現在のバージョン**: Phase 0 (0.1.0)

**セマンティックバージョニング**:
- Phase 0: `0.1.x`（コアライブラリ）
- Phase 1: `0.2.x`（GUI 統合）
